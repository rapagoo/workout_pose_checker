...
닫기를 사용하면 웹캠 장치가 해제됩니다. 현재 UI는 테스트를 위해
`MockPoseService`와 연결되어 실제 자세와 관계없이 상태와 성공 횟수를
갱신합니다. 웹캠 영상은 계속 UI에 표시됩니다. 시간 모드는 설정한 시간이 끝나면,
횟수 모드는 성공 횟수가 목표에 도달하면 운동 결과 페이지로 이동합니다.
실제 모델로 복귀할 때는 `main_window.py`의 `USE_MOCK_POSE_SERVICE`를
`False`로 변경합니다.
